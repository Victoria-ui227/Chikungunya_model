...
pacman::p_load(deSolve, ggplot2,gridExtra, reshape2, dplyr)

#Function

new_chikungunya_model<-function(t,y,parms){
  with(as.list(c(y, parms)),{
    
    #Age-specific compartments
    S1<-y[1];E1<-y[2] #Age group 1(<5 years): Susceptible, Exposed
    S2<-y[3];E2<-y[4] #Age group 2(5+ years): Susceptible, Exposed
...
run_chikungunya_simulation <- function(params, initial_conditions, time_points) {
  ...
  # Add column names
  colnames(out_df) <- c("time", "S1", "E1", "S2", "E2", "Im", "Is", "C", "R", 
                        "Sm", "Em", "Im_mosq")
...
plot_chikungunya_results <- function(results) {
  ...
  age_specific_data <- results |>
    ...
    reshape2::melt(id.vars = "time", variable.name = "Compartment", value.name = "Population")
  ...
  epi_data <- results |>
    ...
    reshape2::melt(id.vars = "time", variable.name = "Indicator", value.name = "Population")
...
main_simulation <- function() {
  ...
  params <- list(
    # Human parameters
    mu = 1/(62*365),              # Human natural death rate based on the kenyan life expectancy
    sigma = 1/4,                  # 1/incubation period (days)-(4 to 7 days incubation period)
    gamma_m = 1/7,                # Recovery rate from mild infection
    gamma_s = 1/14,               # Recovery rate from severe infection
    gamma_c = 1/30,               # Recovery rate from chronic state (slower)-most people report resolution after 1 month
    severe_prob_1 = 0.3,          # Probability of severe disease in age group 1
    severe_prob_2 = 0.1,          # Probability of severe disease in age group 2
    chronic_prob_m = 0.006,        # Probability of chronic from mild infection-hospitalization ranges from 0.6% to 13%
    chronic_prob_s = 0.013,        # Probability of chronic from severe infection
    ...
    prop_age1_Im = 0.024,           # Proportion of mild infectious in age group 1
    prop_age1_Is = 0.074,           # Proportion of severe infectious in age group 1  
    prop_age1_C = 0.75,           # Proportion of chronic in age group 1
    prop_age1_R = 0.8,            # Proportion of recovered in age group 1
    
    # Mosquito parameters
    Lambda_m = 0.005,              # Mosquito recruitment rate
  ...
  # Initial conditions
  N1_initial <- 2417407  # Initial population age group 1
  N2_initial <- 10241396  # Initial population age group 2
  
  initial_conditions <- c(
    S1 = N1_initial,   # Susceptible age group 1
    E1 = 0,                  # Exposed age group 1
    S2 = N2_initial,     # Susceptible age group 2
    E2 = 0,                  # Exposed age group 2
    Im = 41322,                  # Infectious mild (mixed ages)
    Is = 70979,                  # Infectious severe (mixed ages)
    C = 0,                   # Chronic
    R = 0,                   # Recovered  
    Sm = 23406,               # Susceptible mosquitoes
    Em = 12920,                # Exposed mosquitoes
    Im_mosq = 11046            # Infectious mosquitoes
  )
  
  # Time points
  time_points <- seq(0, 365)  # 1 year simulation
...
analyze_chronic_burden <- function(results) {
  ...
  p_chronic <- ggplot(chronic_data, aes(x = time)) +
    geom_line(aes(y = chronic_cases, color = "Chronic Cases"), size = 1.2) +
    geom_line(aes(y = chronic_prevalence * 1000, color = "Prevalence (per 1000)"), size = 1.2) +
    scale_y_continuous(
      name = "Chronic Cases",
